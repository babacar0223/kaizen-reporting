...
export default function HistoPage() {
  ...
  const chartData = entites.map(e => {
    const annualTarget = (rows as any[]).find(r => r.entite === e && r.lignePl === selectedLigne && r.typeValeur === 'TARGET')?.montant || 0;
    return {
      name: e,
      Actuals: (rows as any[]).find(r => r.entite === e && r.lignePl === selectedLigne && r.typeValeur === 'ACTUALS')?.montant || 0,
      Target: (annualTarget / 12) * nMonths,
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-400 text-center">Vert = Actuals &gt; Target · Rouge = Actuals &lt; Target · Budget proraté sur {nMonths} mois</p>
    </div>
  );
}
