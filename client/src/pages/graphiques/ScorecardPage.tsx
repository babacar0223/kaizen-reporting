...
export default function ScorecardPage() {
  ...
  const buScores = BU_LIST.map((bu, i) => {
    ...
    const revTgt = kpis['Revenue']?.['TARGET'] || 0;
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-base font-semibold text-gray-800">Scorecard BU — Synthèse Direction</h2>
        <p className="text-xs text-gray-400">YTD {moisLabel(mois)} {annee}</p>
      </div>
          ...
          <div key={bu.key} className={`bg-white rounded-xl border-2 shadow-sm p-5 ${bu.isAlert ? 'border-red-300' : 'border-gray-200'}`}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400">{moisLabel(mois)} {annee}</p>
              </div>
