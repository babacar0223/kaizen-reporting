...
export default function CourbePage() {
  ...
  const chartData = MOIS_FR.map((m, i) => {
    ...
    const target = rows.find(r => r.mois === moisNum && r.lignePl.nom === selectedLigne && r.typeValeur === 'TARGET')?.montant || null;
    const n1 = rows.find(r => r.mois === moisNum && r.lignePl.nom === selectedLigne && r.typeValeur === 'YTD_N1')?.montant || null;
    return { name: m, 'Actuals 2026': actuals, 'Target 2026': target, 'Actuals 2025': n1 };
  });

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4 flex-wrap">
        <h2 className="text-base font-semibold text-gray-800">Courbe Mensuelle</h2>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={chartData} margin={{ top: 10, right: 20, left: 20, bottom: 5 }}>
            ...
            <Legend />
            <Line type="monotone" dataKey="Actuals 2026" stroke="#00A3B4" strokeWidth={2.5} dot={{ r: 3 }} connectNulls={false} />
            <Line type="monotone" dataKey="Target 2026" stroke="#E8A000" strokeWidth={1.5} strokeDasharray="5 5" dot={false} connectNulls={false} />
            <Line type="monotone" dataKey="Actuals 2025" stroke="#94a3b8" strokeWidth={1.5} dot={false} connectNulls={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
